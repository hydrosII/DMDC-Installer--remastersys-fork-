...
###################1
progressbar () {
tail -f /opt/dmdc-locales/es_ES/dmdc-installer | $DIALOG $TITLE"$TITLETEXT" $TEXT"$@" --no-buttons --progress --pulsate --auto-close
}
 
###################0
GPARTEDISK ()  {
$DIALOG $TITLE"$TITLETEXT" --button=Exit:2 --button=Continue:1 --button="Create Partition":0 $TEXT"<b>gparted</b> also allow you to create new partitions if necessary to install the system. \nIf you already have partitions to install the system, press <b>Continue</b>.  "
QUES=$?
if [ $QUES = 2 ]; then
exit 0
fi
...
lsblk -l -o NAME,TYPE,SIZE,FSTYPE,GROUP | grep -e disk -e floppy | grep -v loop | grep -v cdrom | grep -v $LIVECD | grep -v part | grep -v swap | sed "s/        \+/ '-----' /g" | awk '{print $1, $3, $2, $5, $4}' >> /tmp/disks
...
do
PARTDRIVE=`$DIALOGMENU --height=250 $TITLE"$TITLETEXT" --button="Exit":1 --button="OK":0 --list --column=Disk --column="Size(GB)" --column=Tipe --column="Location"  --column=Infom --no-click $TEXT"Please select a drive on which to create new partitions <b>Gparted</b>." $diskmenu`
QUES="$?"
done
if [ $QUES != 0 ]; then
exit 0
...
$DIALOG $TITLE"$TITLETEXT" --button="Exit":2 --button="No":1  --button="yes":0 $TEXT"The installer will use the locale, the language and keyboard layout by default. \nDid you install or remove other keyboard options and location?. "
QUES=$?
if [ $QUES = 0 ]; then
...
###################0
QUES=""
$DIALOG $TITLE"$TITLETEXT" --button="Exit":3 --button="Use SWAP existing":2 --button="Not used SWAP":1 --button="Create SWAP":0 $TEXT"If your computer is low on RAM (less than 2GB) is convenient and appropriate to use a swap partition. \nIf you do not have it you can create one by pressing the appropriate button. \nWhat would you like to do?. "
...
lsblk -l -o NAME,TYPE,SIZE,FSTYPE,GROUP | grep -e disk -e floppy | grep -v loop | grep -v cdrom | grep -v $LIVECD | grep -v part | grep -v swap | sed "s/        \+/ '-----' /g" | awk '{print $1, $3, $2, $5}' >> /tmp/disks
...
do
PARTDRIVESWAP=`$DIALOGMENU --height=250 $TITLE"$TITLETEXT"--button="Exit":1 --button="OK":0  --list --column=Disk --column="Size(GB)" --column=Tipe --column=Tipe --no-click $TEXT"Please select a unit \nwhich to create the partition <b>Swap</b>.  " $diskmenuswap`
QUES="$?"
done
...
until [[ "$SWAP" !=  "" ||  "$QUES" =  "1" ]]
do
SWAP=`$DIALOGMENU --height=250 $TITLE"$TITLETEXT" --button="Exit":1 --button="OK":0  --list --column=Part --column="Size(GB)" --column="Partition type" --column="Location"  --no-click $TEXT"Please select a swap partition to use.  " $diskswapmenu`
QUES="$?"
done
...
###################0
lsblk -l -o NAME,SIZE,TYPE,FSTYPE,GROUP | grep -e part -e floppy | grep -v loop | grep -v swap  | grep -v cdrom | grep -v $LIVECD | grep part | sed "s/        \+/ '-----' /g" | awk '{print $1, $2, $3, $4, $5}' >> /tmp/diskpart
...
do
TARGETPART=`$DIALOGMENU --height=350 $TITLE"$TITLETEXT" --button="Exit":1 --button="OK":0  --list --column=Part --column=Size --column=Tipe --column="FsTipe" --column="Location"  --no-click $TEXT"Please select a partition to install the system.  " $diskpartmenu`
QUES="$?"
done
...
lsblk -l -o NAME,SIZE,TYPE,FSTYPE,GROUP | grep -e part -e floppy | grep -v loop | grep -v swap  | grep -v cdrom | grep -v $TARGETPART | grep -v $LIVECD | grep part | sed "s/        \+/ '-----' /g" | awk '{print $1, $2, $3, $4, $5}' >> /tmp/diskparth
...
[[ "$TARGETUSERFULLNAME" != "${TARGETUSERFULLNAME//[\' ]/}" ]] && $DIALOG $TITLE"$TITLETEXT" $MSGBOX $TEXT"The real user name can not contain spaces or special characters. "
 
[ "$TARGETUSER" = "" ] && $DIALOG $TITLE"$TITLETEXT" $MSGBOX $TEXT"Username is blank. "
 
[[ "$TARGETUSER" != "${TARGETUSER//[\' ]/}" ]] && $DIALOG $TITLE"$TITLETEXT" $MSGBOX $TEXT"The username can not contain spaces or special characters. "
...
[[ "$TARGETHOSTNAME" != "${TARGETHOSTNAME//[\' ]/}" ]] && $DIALOG $TITLE"$TITLETEXT" $MSGBOX $TEXT"The computer name can not contain spaces or special characters. "
...
lsblk -l -o NAME,SIZE,TYPE,FSTYPE,GROUP | grep -e disk -e floppy -e part | grep -v loop | grep -v cdrom | grep -v $LIVECD | grep -v swap | sed "s/        \+/ '-----' /g" | awk '{print $1, $2, $3, $4, $5}' >> /tmp/diskgrub
...
#Timezone setting
QUES=""
$DIALOG $TITLE"$TITLETEXT" --button="Exit":2  --button="No":1 --button="yes":0 $TEXT"Is your system clock set to your current local time?. \nAnswering no will indicate it is set to UTC. "
QUES=$?
if [ $QUES = 2 ]; then
exit 0
fi
...
do
ZONESINFO=`$DIALOGMENU --width=400 --height=600 $TITLE"$TITLETEXT" --button="Exit":1 --button="OK":0 --list --column=Zone --column="Day" --column=Time  $TEXT"Please select a timezone for your system. " $ZONES`
QUES="$?"
done
...
FNTARGETPART=`lsblk -l -o NAME,SIZE,TYPE,FSTYPE,GROUP | grep -e part -e floppy | grep -v loop | grep -v swap  | grep -v cdrom | grep -v $LIVECD | grep part | awk '{print $1}'`
FNGRUB=`lsblk -l -o NAME,TYPE,SIZE,GROUP,FSTYPE | grep -e disk -e floppy -e part | grep -v loop | grep -v cdrom | grep -v $LIVECD | grep -v swap | awk '{print $1}'`
...
hb6=""
QUES=""
while [[ "$hb3" != "${hb3//[\' ]/}" || "$hb4" != "${hb4//[\' ]/}" || "$hb7" != "${hb7//[\' ]/}" || "$hb3" = "" ||  "$hb4" = "" ||  "$hb7" = "" ]]
do
CONFIR=`$DIALOG --align=right --columns=2 $TITLE"$TITLETEXT" --button="Abort":1 --button="Install":0 $TEXT"\nPlease verify that the information is correct. If you intend to modify some data, make sure you do it correctly. " \
--form \
--field=:LBL "" \
...
if [[ $hb3 != $TARGETUSERFULLNAME || $hb4 != $TARGETUSER || $hb5 != $AUTOINI || $hb6 != $USERSUDO || $hb7 != $TARGETHOSTNAME || $hb10 != $ZONESINFO || $hb16 != $TARGETPART || $hb17 != $FSTYPES || $hb19 != $HOMEPART || $hb20 != $HFSTYPE || $hb22 != $SWAP || $hb23 != $YESNOSWAP || $hb25 != $SPLASH || $hb26 != $GRUBSELT ]]; then
QUES=""
$DIALOG $TITLE"$TITLETEXT" --button="Abort":1 --button="Install":0  $TEXT"There have been changes in the above data. \nIf you are certain changes press <b>Install</b>."
QUES="$?"
if [ $QUES != 0 ]; then
exit 0
fi
...
sed -i -e 's/AutomaticLogin=dmdc/AutomaticLogin='$TARGETUSER'/g' -e 's/AutomaticLoginEnable=true/AutomaticLoginEnable=false/g' /tmp/mdm.conf
fi
...
UUID=`blkid -s UUID -o value /dev/$TARGETPART` / $FSTYPES relatime,errors=remount-ro 0 1
# /dev/$HOMEPART home
UUID=`blkid -s UUID -o value /dev/$HOMEPART` /home `lsblk -l -o FSTYPE /dev/$HOMEPART | grep -v FSTYPE | awk '{print $1}'` relatime 0 0
# /dev/$SWAP
UUID=`blkid -s UUID -o value /dev/$SWAP` none swap sw 0 0
# cdrom
$TARGETCDROM /media/cdrom udf,iso9660 user,noauto,exec,utf8 0 0
